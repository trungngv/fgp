function out = spgp_lik_both(Ntrain,w,ind,n,del)
...
%
% ind -- indice of training inputs and targets
...
global globalx;
global globaly;
if (Ntrain ~= size(globalx,1)) % mainly for slave
  load_global_vars();
end
x = globalx(ind,:);
y = globaly(ind);
...
% Likelihood
out.fw = sum(log(diag(Lm))) + (N-n)/2*log(sig) + ... 
...
out.dfw = [reshape(dfxb,n*dim,1);dfb;dfc;dfsig];
fprintf('O');
