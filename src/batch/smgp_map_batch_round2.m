...
output_file = 'fagpe/output/msgpmapv2_k2maha_round2.mat';
try
...
for K=[2]
  for sss = 3:4
    ...
    for idata = 1:length(dataset_names)
      ...
      logger.(tstamp).(name).version = 2;
      logger.(tstamp).(name).K = K;
      logger.(tstamp).(name).num_basis = nu;
      logger.(tstamp).(name).func_assign = func_assign;
      logger.(tstamp).(name).max_iters = max_iters;
      logger.(tstamp).(name).hyp_iters = hyp_iters;
