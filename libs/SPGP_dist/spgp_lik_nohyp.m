...
% SPGP_LIK_NOHYP [fw,dfw] = spgp_lik_nohyp(w,y,x,n,del)
% neg. log likelihood for SPGP and gradients with respect to
...
%      hyp(1:dim) = log( b ) = -2*log(ell)
%      hyp(dim+1) = log(sf^2)
%      hyp(dim+2) = log(sig^2)
%
%      where cov = (sf^2) * exp[-0.5 * sum_d b_d * (x_d - x'_d)^2] 
%                       + sig^2 * delta(x,x')
%
